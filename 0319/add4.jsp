...
<div class="container">
  ...
  <div class="content">
    <h1>新增!</h1>
    <p>
    <%@ page contentType="text/html; charset=utf-8" language="java" import="java.sql.*,java.text.SimpleDateFormat,java.util.Date" errorPage="" %>
    <% 
//取得傳入參數
String eNo = request.getParameter("no").trim();
String eName = new String(request.getParameter("name").getBytes("iso-8859-1"), "utf-8").trim();
String eBirthday = request.getParameter("birthday");

//將字串日期轉成java.sql.Date, 以便寫入資料庫
java.sql.Date sqlBirthday=null;

try{
	SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
	format.setLenient(false);
	Date parsed = format.parse(eBirthday);
	sqlBirthday = new java.sql.Date(parsed.getTime());
}catch(Exception e){
	out.print("日期錯誤, 請檢查");
	return;
...
//建立SQL命令物件
PreparedStatement stmt = con.prepareStatement("insert into employee(empNo, empName, birthday) values (?, ?, ?)"); 
stmt.setString(1, eNo); 
stmt.setString(2, eName);
stmt.setDate(3, sqlBirthday);
...
try{ 
	...
	out.println("無法寫入, 可能是客戶編號重覆, 或是寫入資料格式有誤");
	stmt.close(); 
	con.close(); 
	return;
}
